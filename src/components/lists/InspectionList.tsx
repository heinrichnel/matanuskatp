...
const InspectionList: React.FC<InspectionListProps> = ({ status }) => {
  ...
  // Update filterStatus when status prop changes
  useEffect(() => {
    if (status) {
  ...
  const filteredInspections = inspections.filter((inspection) => {
    ...
    // Map 'active' status to non-completed statuses
    const statusFilter =
      status === "active"
        ? ["pending", "in_progress", "requires_action"]
        : status === "completed"
          ? ["completed", "failed"]
          : null;
    ...
    const matchesStatus = !statusFilter
      ...
      : statusFilter.includes(inspection.status);
