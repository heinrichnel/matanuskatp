  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      results = results.filter(po => 
        ...
        po.items.some(item => item.name.toLowerCase().includes(query) || item.sku.toLowerCase().includes(query))
      );
    }
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {isLoading ? (
                  <tr>
                    <td colSpan={8} className="px-6 py-4 text-center">Loading purchase orders...</td>
                  </tr>
                ) : filteredOrders.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-6 py-4 text-center">No purchase orders found matching your criteria</td>
                  </tr>
                ) : (
                    ...
                    <React.Fragment key={po.id}>
                      <tr className={`hover:bg-gray-50 ${expandedOrder === po.id ? 'bg-blue-50' : ''}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full items-center ${getStatusBadge(po.status)}`}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap font-medium">{formatCurrency(po.totalAmount)}</td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getPaymentBadge(po.paymentStatus)}`}>
                            {po.paymentStatus.charAt(0).toUpperCase() + po.paymentStatus.slice(1).replace('_', ' ')}
                          </span>
                        </td>
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                  ...
                                  <ul className="mt-2 text-sm space-y-1">
                                    <li>Expected Delivery: {new Date(po.expectedDelivery).toLocaleDateString()}</li>
                                    {po.approvedBy && (
                                      <li>Approved By: {po.approvedBy} on {po.approvedDate ? new Date(po.approvedDate).toLocaleDateString() : 'Unknown'}</li>
                                    )}
                                    <li>Created On: {new Date(po.orderDate).toLocaleDateString()}</li>
                                    ...
                                    <table className="min-w-full divide-y divide-gray-200 text-sm">
                                      ...
                                      <tbody className="bg-white divide-y divide-gray-200">
                                        ...
                                        <tr className="bg-gray-50">
                                          ...
                                          <td className="px-4 py-2 font-medium text-right">Order Total:</td>
                                          <td className="px-4 py-2 font-bold">{formatCurrency(po.totalAmount)}</td>
                                        </tr>
                                      </tbody>
                                    </table>
                                  </div>
