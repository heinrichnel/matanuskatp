...
import { AlertTriangle, Calendar, Map, Upload, Truck, Cloud, Save } from 'lucide-react';
...
const IncidentReportForm: React.FC<IncidentReportFormProps> = ({
  ...
  const [validationErrors, setValidationErrors] = useState<Partial<Record<keyof IncidentFormData, string>>>({});
      ...
      <CardContent className="p-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="incidentType">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="incidentArea">
                Incident Area
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="weatherCondition">
                Weather Condition
              </label>
              <div className="relative">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="severityRating">
                Severity Rating
              </label>
                ...
                {['Low', 'Medium', 'High', 'Critical'].map((rating) => (
                  <label key={rating} className="flex items-center justify-center">
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`w-full text-center py-2 rounded cursor-pointer ${
                      formData.severityRating === rating
                        ? rating === 'Low' ? 'bg-green-500 text-white'
                          : rating === 'Medium' ? 'bg-yellow-500 text-white'
                          : rating === 'High' ? 'bg-orange-500 text-white'
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="vehicleActivity">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-4 mb-4">
              ...
              {formData.imageUrls?.map((url, index) => (
                <div key={`prev-${index}`} className="relative w-24 h-24 border rounded overflow-hidden">
                  <img
                    src={url}
            ...
            <p className="text-xs text-gray-500">
              Upload up to 6 images showing the incident scene and damage. Supported formats: JPG, PNG, GIF.
            </p>
          </div>
