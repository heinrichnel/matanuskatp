...
const InspectionForm: React.FC<InspectionFormProps> = ({
  ...
  const [formState, setFormState] = useState({
    ...
    items: existingInspection?.items || template.items.map((item: any) => ({
      ...item,
      status: 'pending',
      notes: ''
    }))
  ...
  const handleItemStatusChange = (itemId: string, status: 'pass' | 'fail' | 'pending', notes: string) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">{template.name} • {template.description}</p>
        </div>
          ...
          <TabsList className="px-4 pt-4 bg-transparent border-b w-full flex overflow-x-auto">
            {template.categories.map((category: string) => (
              <TabsTrigger key={category} value={category} className="flex items-center gap-2">
                ...
                <span>{category}</span>
                {itemStats.failed > 0 && formState.items.some((item: any) => 
                  item.category === category && item.status === 'fail'
                ) && (
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-4 p-4 bg-red-50 rounded-lg border border-red-200">
              ...
              <p className="text-sm text-red-700 mt-2"><strong>Notes:</strong> {jobCardItem.notes}</p>
            </div>
