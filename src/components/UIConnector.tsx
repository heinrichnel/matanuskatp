...
const UIConnector: React.FC = () => {
  ...
  const removeHighlights = () => {
    document.querySelectorAll('.ui-connector-highlight-button, .ui-connector-highlight-no-handler, .ui-connector-highlight-form')
      .forEach((element) => {
        (element as HTMLElement).classList.remove('ui-connector-highlight-button', 'ui-connector-highlight-no-handler', 'ui-connector-highlight-form');
      });
    
    document.querySelectorAll('.ui-connector-tag')
  ...
  const connectionPercentage = status.totalElements > 0 
        ...
        <div style={{
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              backgroundColor: connectionPercentage > 75 ? '#4CAF50' : connectionPercentage > 50 ? '#FFC107' : '#f44336',
          ...
          <p>Buttons: {status.details.buttons.connected}/{status.details.buttons.total}</p>
          <p>Forms: {status.details.forms.connected}/{status.details.forms.total}</p>
          <p>Inputs: {status.details.inputs.connected}/{status.details.inputs.total}</p>
          ...
          <div style={{ marginTop: '15px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ width: '15px', height: '15px', backgroundColor: '#f44336', marginRight: '5px' }}></div>
