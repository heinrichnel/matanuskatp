...
import { Tyre, TyreSize, TyreInspectionEntry } from '../../types/workshop-tyre-inventory';
...
const TyreInspection: React.FC = () => {
  ...
  // Position options state - used to track available positions for the selected fleet
  ...
  const handleTyreSelect = (tyre: Tyre | null) => {
    ...
    if (tyre) {
      ...
      setFormData(prevData => ({
        ...
        currentMileage: tyre.currentMileage?.toString() || tyre.installDetails.mileage.toString(),
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.inspector || !formData.fleetNumber || !formData.tyrePosition ||
    ...
    try {
      ...
      const status =
        formData.detected.treadDepthIssue && parseFloat(formData.treadDepth) < TYRE_THRESHOLDS.treadDepth.worn ? 'urgent' :
              ...
              {showInspectionHistory ? (
                ...
                <div className="space-y-4">
                  <div className="p-3 bg-gray-50 rounded-md">
                    ...
                    <div className="flex items-center">
                      <p className="text-sm">
                        Position: <span className="font-medium capitalize">{selectedTyre.installDetails.position.replace(/-/g, ' ')}</span>
                      </p>
                      ...
                      <div className="space-y-3 mb-4">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <Input
                            ...
                            onChange={(value) => handleChange('tyreSize', { ...formData.tyreSize, width: parseInt(value, 10) || 0 })}
                            placeholder="e.g., 315"
                          />
                          <Input
                            label="Aspect Ratio"
                            ...
                            onChange={(value) => handleChange('tyreSize', { ...formData.tyreSize, aspectRatio: parseInt(value, 10) || 0 })}
                            placeholder="e.g., 80"
                          />
                          <Input
                            label="Rim Diameter"
                            ...
                            onChange={(value) => handleChange('tyreSize', { ...formData.tyreSize, rimDiameter: parseFloat(value) || 0 })}
                  ...
                  {hasIssues && (
                    <div className="bg-amber-50 border-l-4 border-amber-500 p-3">
                      ...
                      {formData.detected.treadDepthIssue && parseFloat(formData.treadDepth) < TYRE_THRESHOLDS.treadDepth.worn && (
