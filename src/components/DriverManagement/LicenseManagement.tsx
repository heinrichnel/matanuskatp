...
import { FileText, Plus, Upload, Download, Filter, Edit, Trash2, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
...
const mockLicenses = [
  {
    ...
    documents: ['license-front.jpg', 'license-back.jpg']
  },
  {
    id: 'lic-002',
    driverId: 'drv-002',
  ...
  },
  {
    ...
    licenseNumber: 'GH-DL-901234',
    type: 'Commercial',
    issueDate: '2020-07-25',
...
const LicenseManagement: React.FC = () => {
  ...
  const filteredLicenses = filterStatus === 'all' 
    ? licenses 
    : licenses.filter(license => license.status === filterStatus);
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track and manage driver licenses, renewals, and compliance</p>
        </div>
        ...
        <CardContent>
          ...
          <div className="mt-4 flex justify-between items-center">
            <p className="text-sm text-gray-700">
              Showing <span className="font-medium">1</span> to <span className="font-medium">{filteredLicenses.length}</span> of{' '}
          ...
          <CardContent>
            <ul className="space-y-3">
              {licenses
                ...
                .map(license => (
                  <li key={license.id} className="flex items-center justify-between p-3 border rounded-md">
                    <div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-lg font-bold text-blue-600">
                  {Math.round((licenses.filter(l => l.status === 'active').length / licenses.length) * 100)}%
                </span>
              </div>
              <div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${(licenses.filter(l => l.status === 'active').length / licenses.length) * 100}%` }}
                  ></div>
                </div>
              </div>
              <div className="mt-6">
