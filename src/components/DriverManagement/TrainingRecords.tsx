...
import { Search, Filter, Calendar, Clock, CheckCircle, User } from 'lucide-react';
...
const TrainingRecords: React.FC = () => {
  ...
  const filteredTrainings = selectedStatus === 'all' 
    ? trainings 
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'completed':
        return <span className="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800 flex items-center"><CheckCircle className="h-3 w-3 mr-1" /> Completed</span>;
      case 'scheduled':
        return <span className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800 flex items-center"><Calendar className="h-3 w-3 mr-1" /> Scheduled</span>;
      case 'in-progress':
        return <span className="px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-800 flex items-center"><Clock className="h-3 w-3 mr-1" /> In Progress</span>;
      case 'expired':
                  ...
                  <tr key={training.id}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-3">
                          <div className="text-sm font-medium text-gray-900">{training.driverName}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-800">{training.type}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
              ...
              .map(training => (
                <div key={`upcoming-${training.id}`} className="border rounded-lg p-4">
                  ...
                  <div className="flex items-center text-sm text-gray-500 mb-4">
                    ...
                    <span>{new Date(training.scheduledDate).toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={`text-xs ${training.required ? 'text-red-600' : 'text-gray-500'}`}>
