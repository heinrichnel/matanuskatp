      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {filteredTyres.length} unique items
                    </p>
                  </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredTyres.map((tyre) => (
                      <React.Fragment key={tyre.id}>
                        <tr className={`hover:bg-gray-50 ${tyre.quantity <= tyre.reorderLevel ? 'bg-red-50' : ''}`}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <button 
                              className="flex items-center"
                              onClick={() => toggleExpand(tyre.id)}
                            >
                          ...
                          <td className={`px-6 py-4 whitespace-nowrap text-sm text-center ${
                            tyre.quantity <= tyre.reorderLevel ? 'text-red-600 font-bold' : 'text-gray-900'
                          }`}>
                            {tyre.quantity}
                          </td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-center">
                            <span className={`px-2 py-1 inline-flex text-xs leading-5 font-medium rounded-full ${
                              tyre.status === 'in_stock' ? 'bg-green-100 text-green-800' :
                              tyre.status === 'on_order' ? 'bg-yellow-100 text-yellow-800' :
                              tyre.status === 'backordered' ? 'bg-orange-100 text-orange-800' :
                            ...
                            <td colSpan={8} className="px-6 py-4 bg-gray-50">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                  ...
                                  <p className="text-sm text-gray-600">Brand: <span className="font-medium">{tyre.brand}</span></p>
                                  <p className="text-sm text-gray-600">Model: <span className="font-medium">{tyre.model || '-'}</span></p>
                                  <p className="text-sm text-gray-600">Pattern: <span className="font-medium">{tyre.pattern || '-'}</span></p>
                                  <p className="text-sm text-gray-600">Size: <span className="font-medium">{tyre.size}</span></p>
                                </div>
                                <div>
                                  <h4 className="text-sm font-medium text-gray-900 mb-1">Inventory</h4>
                                  <p className="text-sm text-gray-600">Quantity: <span className="font-medium">{tyre.quantity}</span></p>
                                  <p className="text-sm text-gray-600">Min Stock: <span className="font-medium">{tyre.reorderLevel}</span></p>
                                  <p className="text-sm text-gray-600">Location: <span className="font-medium">{tyre.storeLocation}</span></p>
                                  <p className="text-sm text-gray-600">Status: <span className="font-medium">{tyre.status.replace('_', ' ').toUpperCase()}</span></p>
                                </div>
                                <div>
                                  <h4 className="text-sm font-medium text-gray-900 mb-1">Supplier</h4>
                                  <p className="text-sm text-gray-600">Supplier: <span className="font-medium">{tyre.supplier.name}</span></p>
                                  <p className="text-sm text-gray-600">Cost: <span className="font-medium">{formatCurrency(tyre.cost, 'ZAR')}</span></p>
                                  <p className="text-sm text-gray-600">Total Value: <span className="font-medium">{formatCurrency(tyre.cost * tyre.quantity, 'ZAR')}</span></p>
                                  <div className="mt-2">
        ...
        <TabsContent value="position" className="mt-6">
          <div className="bg-white rounded-lg p-6 shadow">
            ...
            <p className="text-gray-600 mb-6">
              The position mapping view shows the current tyres installed on each fleet vehicle, their condition, and history.
            </p>
        ...
        <TabsContent value="inspection" className="mt-6">
          <div className="bg-white rounded-lg p-6 shadow">
            ...
            <p className="text-gray-600 mb-6">
              Track tyre inspections, including tread depth measurements, pressure checks, and wear patterns.
            </p>
        ...
        <TabsContent value="analytics" className="mt-6">
          <div className="bg-white rounded-lg p-6 shadow">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <h4 className="text-md font-medium text-gray-800 mb-3">Optimization Recommendations</h4>
              <ul className="list-disc pl-5 space-y-2 text-sm text-gray-600">
                <li>Consider increasing order quantities of Perelli tyres due to their superior cost per kilometer performance.</li>
                <li>Evaluate tyre rotation practices to improve steer position lifespan.</li>
                <li>Schedule training for technicians on proper tyre installation to reduce premature failures.</li>
                <li>Implement regular tread depth monitoring to identify abnormal wear patterns early.</li>
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-yellow-700">
                    <ul className="list-disc pl-5 space-y-1">
                      ...
                      <li>Fleet 1T (positions 5, 6, 7, 8) - Due within 30 days based on wear rate</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Button size="xs" variant="outline">Schedule</Button>
                    </td>
                  </tr>
                  <tr className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
