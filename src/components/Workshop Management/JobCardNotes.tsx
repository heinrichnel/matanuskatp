...
const JobCardNotes: React.FC<JobCardNotesProps> = ({
  ...
  const [noteType, setNoteType] = useState<'general' | 'technician' | 'customer' | 'internal'>('general');
  ...
  const getNoteTypeLabel = (type: string) => {
    switch (type) {
      case 'technician': return 'Technician Note';
      case 'customer': return 'Customer Note';
            ...
            {notes.map(note => (
              <div 
                ...
                className={`p-4 rounded-lg border ${
                  note.type === 'technician' ? 'bg-blue-50 border-blue-200' :
                  note.type === 'customer' ? 'bg-purple-50 border-purple-200' :
                  note.type === 'internal' ? 'bg-orange-50 border-orange-200' :
                ...
                {editingNoteId === note.id ? (
                  ...
                  <div>
                    <div className="flex justify-between items-start">
                      <div>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getNoteTypeClass(note.type)}`}>
