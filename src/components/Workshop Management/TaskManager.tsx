...
const TaskManager: React.FC<TaskManagerProps> = ({
  ...
  const calculateCompletionStats = () => {
    ...
    const completionPercentage = total > 0 
      ? ((completed + verified + notApplicable) / total) * 100 
      : 0;
                      ...
                      <div>
                        ...
                        {task.status === 'completed' && task.completedBy && task.completedAt && (
                          <p className="text-sm text-green-600 mt-1">
                            <strong>Completed by:</strong> {task.completedBy} on {formatDateTime(task.completedAt)}
                          </p>
                        )}
                      ...
                      <div className="mt-3 pt-3 border-t flex justify-between items-center">
                        <div className="flex items-center">
                          ...
                          <select
                            ...
                            value={task.status}
                            onChange={(e) => handleStatusChange(task.id, e.target.value as JobCardTask['status'])}
                          >
            ...
            <div className="space-y-4">
              ...
              <div className="divide-y divide-gray-200">
                {getTaskHistory(selectedTaskId).length > 0 ? (
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium text-gray-900">
                            {entry.event === 'statusChanged' && (
                              <>
                                Status changed from <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${getStatusClass(entry.previousStatus || '')}`}>{entry.previousStatus}</span> to <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${getStatusClass(entry.newStatus || '')}`}>{entry.newStatus}</span>
                              </>
                            )}
                            {entry.event === 'assigned' && (
                              <>Task assigned to {entry.newStatus}</>
                            )}
                  ...
                  <p className="py-4 text-center text-gray-500">No history available for this task</p>
                )}
              </div>
            </div>
          </div>
