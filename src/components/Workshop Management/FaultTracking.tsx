...
const FaultTracking: React.FC = () => {
  ...
  const criticalFaults = mockFaults.filter(f => f.severity === 'critical' && f.status !== 'resolved').length;
              ...
              {filteredFaults.map(fault => (
                <div 
                  ...
                  className={`p-4 rounded-lg border ${
                    fault.severity === 'critical' ? 'border-l-4 border-l-red-500' :
                    fault.severity === 'high' ? 'border-l-4 border-l-orange-500' :
                    ''
                  }`}
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getSeverityClass(fault.severity)}`}>
                          {fault.severity.toUpperCase()}
                        </span>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusClass(fault.status)}`}>
                          {fault.status === 'in_progress' ? 'IN PROGRESS' : fault.status.toUpperCase()}
                        </span>
                      </div>
                    ...
                    <div className="flex space-x-2">
                      ...
                      {fault.status !== 'resolved' && (
                        <Button
                          size="sm"
                          variant={fault.status === 'open' ? 'primary' : 'outline'}
                        >
