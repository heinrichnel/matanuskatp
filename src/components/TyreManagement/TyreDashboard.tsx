...
const TyreDashboard: React.FC = () => {
  ...
  const handleExportCSV = () => {
    if (showInventory) {
      ...
      const fields = [
        'id', 'brand', 'model', 'size', 'serialNumber', 'dotCode',
        'installDetails.vehicle', 'installDetails.position',
        'installDetails.mileage', 'installDetails.date',
        'treadDepth', 'pressure', 'status', 'cost', 'estimatedLifespan',
        'costPerKm'
      ];
                ...
                {filterCriteria.brand && (
                  <div>
                    ...
                    >
                      ...
                      {TYRE_REFERENCES
                        .filter((ref: TyreReference) => !filterCriteria.brand || ref.brand === filterCriteria.brand)
                        .map((ref: TyreReference) => ref.pattern)
                        .filter((pattern: string, index: number, self: string[]) => pattern && self.indexOf(pattern) === index)
                        .map((pattern: string) => (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Brand</th>
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Count</th>
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Avg. Cost</th>
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Avg. Lifespan</th>
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Cost per KM</th>
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ROI Rating</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {Object.entries(brandPerformance).map(([brand, stats]) => (
                    <tr key={brand}>
                      ...
                      <td className="py-3 px-3 whitespace-nowrap">{stats.avgLifespan.toLocaleString()} km</td>
                      <td className="py-3 px-3 whitespace-nowrap">${stats.avgCostPerKm.toFixed(4)}/km</td>
                      <td className="py-3 px-3 whitespace-nowrap">
                        <div className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${stats.avgCostPerKm < 0.013 ? 'bg-green-100 text-green-800' :
                            stats.avgCostPerKm < 0.014 ? 'bg-blue-100 text-blue-800' :
                              stats.avgCostPerKm < 0.015 ? 'bg-yellow-100 text-yellow-800' :
                                'bg-red-100 text-red-800'}`}>
                          {stats.avgCostPerKm < 0.013 ? 'Excellent' :
                            stats.avgCostPerKm < 0.014 ? 'Good' :
                              stats.avgCostPerKm < 0.015 ? 'Average' : 'Poor'}
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="space-y-2">
                  {Array.from(new Set(tyres.map(t => (t as any).tyreSize?.width))).map(width => {
                    if (!width) return null;
                    const count = tyres.filter(t => (t as any).tyreSize?.width === width).length;
              ...
              <div>
                ...
                <div className="space-y-2">
                  {Array.from(new Set(tyres.map(t => (t as any).tyreSize?.aspectRatio))).map(ratio => {
                    if (!ratio) return null;
                    const count = tyres.filter(t => (t as any).tyreSize?.aspectRatio === ratio).length;
                    const percentage = (count / tyres.length) * 100;
                    return (
              ...
              <table className="min-w-full bg-white">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Count</th>
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Avg. Wear Rate</th>
                    <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rotation Recommendation</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {/* Group by position type (front, drive, trailer) */}
                  {['front', 'drive', 'trailer'].map(posType => {
                    const positionTyres = tyres.filter(t => t.installDetails.position.includes(posType));
                    if (positionTyres.length === 0) return null;

                    // Calculate average tread depth
                    const avgTreadDepth = positionTyres.reduce((sum, t) => sum + t.treadDepth, 0) / positionTyres.length;

                    // Get earliest inspection date for these tyres
                    const allDates = positionTyres.flatMap(t => (t as any).inspectionHistory || [])
                    ...
                    let wearRate = 'N/A';
                    if (minDate) {
                      const monthsDiff = (Date.now() - minDate.getTime()) / (1000 * 60 * 60 * 24 * 30);
                      if (monthsDiff > 0) {
                    ...
                    return (
                      <tr key={posType}>
                        <td className="py-3 px-3 whitespace-nowrap capitalize">{posType} Position</td>
                        <td className="py-3 px-3 whitespace-nowrap">{positionTyres.length}</td>
                        <td className="py-3 px-3 whitespace-nowrap">{wearRate}</td>
                        <td className="py-3 px-3 whitespace-nowrap">
                          <div className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                            ${recommendation === 'No action needed' ? 'bg-green-100 text-green-800' :
                              recommendation.includes('Rotate') ? 'bg-yellow-100 text-yellow-800' :
                                'bg-red-100 text-red-800'}`}>
                            {recommendation}
                          </div>
                        </td>
                    ...
                    <div className="mt-2">
                      <div className="mb-4">
                        <p className="text-sm text-gray-500">
                          <strong>Brand:</strong> {selectedTyre.brand} <strong>Model:</strong> {selectedTyre.model}
                        </p>
                        <p className="text-sm text-gray-500">
                          <strong>Size:</strong> {selectedTyre.size} <strong>Serial:</strong> {selectedTyre.serialNumber}
                        </p>
                        <p className="text-sm text-gray-500">
                          <strong>Position:</strong> {selectedTyre.installDetails.position} <strong>Vehicle:</strong> {selectedTyre.installDetails.vehicle}
                        </p>
                      </div>
                      ...
                      <div className="mb-6">
                        ...
                        <div className="h-20 flex items-end space-x-1">
                          {(selectedTyre as any).inspectionHistory?.map((insp: any, index: number) => {
                            const height = Math.min(100, (insp.treadDepth / 10) * 100);
                            return (
                              <div key={index} className="flex flex-col items-center">
                                <div
                                  className={`w-12 rounded-t transition-all ${insp.treadDepth > 6 ? 'bg-green-500' :
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                            <tr>
                              ...
                              <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tread Depth</th>
                              <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Pressure</th>
                              <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sidewall</th>
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {(selectedTyre as any).inspectionHistory?.map((insp: any, index: number) => (
                              <tr key={index}>
                                ...
                                <td className="px-3 py-2 whitespace-nowrap text-sm capitalize">{insp.sidewallCondition?.replace('_', ' ') || 'N/A'}</td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {tyres.map(tyre => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-lg font-medium">{tyre.brand} {tyre.model}</h3>
                  ...
                  <div
                    className={`
                      h-8 w-8 rounded-full flex items-center justify-center
                      ${tyre.status === 'urgent' ? 'bg-red-100 text-red-600' :
                        tyre.status === 'worn' ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-600'}
                    `}
                  >
                    {tyre.status === 'urgent' ? (
                      <AlertTriangle className="h-5 w-5" />
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <div className="flex items-center">
                      <p className={`text-sm font-medium ${tyre.treadDepth < 3 ? 'text-red-600' : tyre.treadDepth < 5 ? 'text-amber-600' : 'text-green-600'}`}>
                        {tyre.treadDepth} mm
                      </p>
                      <div className="w-12 ml-2 bg-gray-200 rounded-full h-1.5">
                        <div
                          className={`h-1.5 rounded-full ${tyre.treadDepth < 3 ? 'bg-red-500' :
                            tyre.treadDepth < 5 ? 'bg-amber-500' : 'bg-green-500'
                            }`}
