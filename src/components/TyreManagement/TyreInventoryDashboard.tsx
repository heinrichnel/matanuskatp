...
import { Search, Filter, Download, Plus, BarChart, PieChart } from 'lucide-react';
...
const TyreInventoryDashboard: React.FC<TyreInventoryDashboardProps> = ({
  ...
  const filteredTyres = tyres
    .filter(tyre => {
      ...
      const matchesManufacturer = filterManufacturer === '' || 
        tyre.manufacturer === filterManufacturer;
      
      const matchesStatus = filterStatus === '' || 
        tyre.status === filterStatus;
      
      const matchesCondition = filterCondition === '' || 
        tyre.condition === filterCondition;
  ...
  const getConditionClass = (condition: string) => {
    switch (condition) {
      ...
      case 'Retreaded': return 'bg-purple-100 text-purple-800';
                ...
                filteredTyres.map((tyre) => (
                  <tr key={tyre.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getConditionClass(tyre.condition)}`}>
                        {tyre.condition}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(tyre.status)}`}>
                        {tyre.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                        <div 
                          className={`h-1.5 rounded-full ${
                            calculateTreadPercentage(tyre.treadDepth) > 70 ? 'bg-green-500' : 
                            calculateTreadPercentage(tyre.treadDepth) > 30 ? 'bg-yellow-500' : 
                            'bg-red-500'
                          }`} 
