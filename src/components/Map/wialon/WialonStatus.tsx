    ...
    <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
      ...
      <div className="p-2 border border-gray-200 dark:border-gray-700 rounded-md mb-2 bg-gray-50 dark:bg-gray-900">
        <div className="flex items-center">
          ...
          <span className="font-medium text-gray-900 dark:text-gray-100">
            {connectionLoading ? 'Checking connection...' : status.connected ? 'Connected' : 'Disconnected'}
          </span>
        </div>
      ...
      {showDetails && status.connected && (
        <div className="text-sm space-y-1 mt-3">
          ...
          {status.serverTime && (
            <div>
              <span className="text-gray-500 dark:text-gray-400 inline-block w-24">Server Time:</span>
              <span className="text-gray-800 dark:text-gray-200">{status.serverTime.toLocaleString()}</span>
            </div>
          )}
          {status.tokenExpiry && (
            <div>
              <span className="text-gray-500 dark:text-gray-400 inline-block w-24">Token Expires:</span>
              <span className={`${new Date() > status.tokenExpiry ? 'text-red-600 dark:text-red-400' : 'text-gray-800 dark:text-gray-200'}`}>
          ...
          {showUnitsCount && (
            <div className="pt-2 mt-2 border-t border-gray-100 dark:border-gray-700">
              ...
              {unitsLoading ? (
                ...
                <span className="text-gray-800 dark:text-gray-200">{units?.length || 0} units available</span>
              )}
            </div>
          )}
        </div>
