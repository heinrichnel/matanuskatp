...
interface PredictiveModel {
  id: string;
  name: string;
  type: 'maintenance' | 'fuelConsumption' | 'customerChurn' | 'routeOptimization' | 'tyreWear' | 'driverRisk';
  description: string;
  accuracy: number;
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        className={`h-2.5 rounded-full ${
                          selectedModel.accuracy > 90 ? 'bg-green-500' : 
                          selectedModel.accuracy > 75 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
                      ...
                      <span 
                        className={`ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${selectedModel.status === 'active' ? 'bg-green-100 text-green-800' : 
                            selectedModel.status === 'training' ? 'bg-blue-100 text-blue-800' : 
                ...
                {insights.map((insight, idx) => (
                  <div key={idx} className={`border-l-4 p-4 ${
                    ...
                    {insight.actionRequired && (
                      <div className="mt-2">
                        <button className="px-3 py-1 bg-white text-sm font-medium rounded border border-gray-300 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                          View Action Plan
                        </button>
                      </div>
                    )}
            ...
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    data={selectedModel.predictions && selectedModel.predictions.length > 0 ? 
                      formatPredictionsForChart(selectedModel.predictions) : mockPredictions}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Scatter 
                      ...
                      data={selectedModel.predictions && selectedModel.predictions.some(p => p.actualValue !== undefined) ? 
                        formatForAccuracyChart(selectedModel.predictions) : mockAccuracyData} 
                      fill="#8884d8" 
                    />
                  </ScatterChart>
                </ResponsiveContainer>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {(selectedModel.predictions || []).slice(0, 10).map((prediction, index) => {
                    const accuracy = prediction.actualValue ? 
                      (1 - Math.abs(prediction.predictedValue - prediction.actualValue) / prediction.actualValue) * 100 : 
                      null;
                            ...
                            <span 
                              className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                ${accuracy > 90 ? 'bg-green-100 text-green-800' : 
                                  accuracy > 75 ? 'bg-yellow-100 text-yellow-800' : 
