...
// ─── Types ───────────────────────────────────────────────────────
import { AdditionalCost, CostEntry, DelayReason, Trip } from "../../types/index";
...
// ─── LIVE TELEMATICS ─────────────────────────────────────────────
import { useWialonUnits } from "../../pages/wialon/hooks/useWialonUnits"; // <--- THIS HOOK PROVIDES TELEMATICS DATA
...
const TripDetails: React.FC<TripDetailsProps> = ({ trip, onBack }) => {
  ...
  const matchedWialonUnit = React.useMemo(() => {
    ...
    if (trip.fleetUnitId) return wialonUnits.find((u) => u.id === Number(trip.fleetUnitId));
    if (trip.fleetNumber) return wialonUnits.find((u) => u.name === trip.fleetNumber);
    return null;
  }, [wialonUnits, trip.fleetUnitId, trip.fleetNumber]);

  // Wialon Vehicle ID support
  const hasWialonTracking = !!trip.fleetUnitId && trip.fleetUnitId !== "";
  const wialonStatus = hasWialonTracking
    ? "Connected to Wialon Unit ID: " + trip.fleetUnitId
    : "Not connected to Wialon";

  // Display unit info
  const wialonUnitDisplay = trip.fleetUnitId
    ? `Unit ID: ${trip.fleetUnitId}`
    : "No tracking unit connected";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {trip.status === "completed" && (
            <Button onClick={() => setShowReport(true)} icon={<Send className="w-4 h-4" />}>
      ...
      <Modal isOpen={showReport} onClose={() => setShowReport(false)} title="Trip Report">
