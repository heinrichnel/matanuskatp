                    ...
                    {loadPlan.cargoItems.map(item => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
                          {LOAD_CATEGORIES.find(cat => cat.value === item.category)?.label || item.category}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
                          {item.quantity}
                        </td>
              ...
              <div className="space-y-4">
                ...
                {loadPlan.cargoItems.some(item => item.hazardous) && (
                  <div className="bg-red-50 p-3 rounded border border-red-200 flex items-start">
                    ...
                    <div>
                      <h4 className="text-sm font-medium text-red-800">Hazardous Materials Present</h4>
                      <p className="text-sm text-red-700 mt-1">
                        This load contains hazardous materials. Ensure all proper documentation is in place and special handling protocols are followed.
                      </p>
                    </div>
                  </div>
                )}
                ...
                {loadPlan.cargoItems.some(item => !item.stackable) && (
                  <div className="bg-yellow-50 p-3 rounded border border-yellow-200 flex items-start">
                    ...
                    <div>
                      <h4 className="text-sm font-medium text-yellow-800">Non-Stackable Items Present</h4>
                      <p className="text-sm text-yellow-700 mt-1">
                        This load contains items that cannot be stacked. Ensure these items are loaded last or on top of other cargo.
                      </p>
                    </div>
                  </div>
                )}
