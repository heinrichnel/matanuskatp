...
const QAReviewPanel: React.FC<QAReviewPanelProps> = ({
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'verified': return <Shield className="w-4 h-4 text-purple-600" />;
      case 'not_applicable': return <Clock className="w-4 h-4 text-gray-600" />;
      ...
      <CardContent>
        <div className="space-y-6">
          ...
          {tasksReadyForVerification > 0 ? (
            <div>
              <h3 className="text-md font-medium text-gray-800 mb-3">Tasks Ready for Verification</h3>
                    ...
                    <div key={task.id} className="bg-white border rounded-lg p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          ...
                          <p className="text-sm text-gray-500 mt-1">{task.category} • {task.estimatedHours}h estimated</p>
                          {task.description && (
                      ...
                      <div className="bg-green-50 border border-green-200 rounded p-2 mb-3">
                        <div className="flex items-center text-sm">
                          ...
                          <span className="text-green-700">
                            Completed by <strong>{task.completedBy}</strong> on {formatDateTime(task.completedAt || '')}
                          </span>
                        </div>
                      </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
              ...
              <h3 className="text-md font-medium text-gray-800 mb-1">No Tasks Ready for Verification</h3>
                        ...
                        <div className="flex-grow">
                          <p className="text-gray-900">
                            ...
                            {entry.event === 'statusChanged' && (
                              <>
                                changed status from <span className="font-medium">{entry.previousStatus}</span> to <span className="font-medium">{entry.newStatus}</span>
                              </>
                            )}
                          ...
                          <p className="text-gray-500 mt-1">Task: {task?.title || 'Unknown Task'}</p>
                        </div>
                      </div>
                    </div>
                  );
