    ...
    const fetchInventoryData = async () => {
      try {
        ...
        const mockData: InventoryDashboardData = {
          ...
          topItems: [
            { name: 'Air Filter XJ-4500', category: 'Filters', stockValue: 12400, quantityInStock: 124, reorderPoint: 40, turnoverRate: 5.8 },
            { name: 'Brake Pads BP-8800', category: 'Braking System', stockValue: 18600, quantityInStock: 62, reorderPoint: 25, turnoverRate: 6.2 },
            { name: 'Engine Oil 15W-40', category: 'Lubricants', stockValue: 14850, quantityInStock: 198, reorderPoint: 75, turnoverRate: 7.4 },
            { name: 'Alternator ALT-520', category: 'Electrical', stockValue: 9860, quantityInStock: 28, reorderPoint: 12, turnoverRate: 3.2 },
            { name: 'Fuel Filter FF-2200', category: 'Filters', stockValue: 8740, quantityInStock: 112, reorderPoint: 45, turnoverRate: 5.6 }
          ],
          vendorPerformance: [
            { vendor: 'AutoParts Plus', onTimeDelivery: 94, qualityScore: 4.6, costVariance: -2.5, avgLeadTime: 3.2 },
            { vendor: 'Truck Parts Ltd', onTimeDelivery: 88, qualityScore: 4.2, costVariance: -1.8, avgLeadTime: 4.5 },
            { vendor: 'Global Supplies', onTimeDelivery: 92, qualityScore: 4.7, costVariance: -3.2, avgLeadTime: 3.8 },
            { vendor: 'FastTruck Components', onTimeDelivery: 86, qualityScore: 4.4, costVariance: -2.1, avgLeadTime: 5.2 },
            { vendor: 'Premium Parts Co.', onTimeDelivery: 96, qualityScore: 4.8, costVariance: -0.8, avgLeadTime: 3.5 }
          ],
          recentDeliveries: [
            { id: 'DEL-4872', vendor: 'AutoParts Plus', date: '2025-07-08', items: 42, value: 12480, status: 'received' },
            { id: 'DEL-4871', vendor: 'Truck Parts Ltd', date: '2025-07-06', items: 28, value: 8650, status: 'partial' },
            { id: 'DEL-4870', vendor: 'Global Supplies', date: '2025-07-05', items: 35, value: 14720, status: 'received' },
            { id: 'DEL-4869', vendor: 'FastTruck Components', date: '2025-07-03', items: 16, value: 5840, status: 'rejected' },
            { id: 'DEL-4868', vendor: 'Premium Parts Co.', date: '2025-07-01', items: 24, value: 9650, status: 'received' }
          ],
          pendingOrders: [
            { id: 'PO-6754', vendor: 'AutoParts Plus', orderDate: '2025-07-05', expectedDelivery: '2025-07-12', items: 38, value: 16250, status: 'processing' },
            { id: 'PO-6753', vendor: 'Global Supplies', orderDate: '2025-07-04', expectedDelivery: '2025-07-11', items: 42, value: 18720, status: 'shipped' },
            { id: 'PO-6752', vendor: 'Truck Parts Ltd', orderDate: '2025-07-02', expectedDelivery: '2025-07-10', items: 24, value: 8950, status: 'delayed' },
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 mb-8">
        <div className="bg-white shadow-md rounded-lg p-4 border-l-4 border-blue-500">
          ...
          <div className="text-2xl font-bold">{dashboardData.summary.totalItems.toLocaleString()}</div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Legend />
                <Line type="monotone" dataKey="received" name="Received" stroke="#8884d8" activeDot={{ r: 8 }} />
                  ...
                  <tr key={item.name} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <span className={`mr-2 ${getStockStatusColor(item.quantityInStock, item.reorderPoint)}`}>●</span>
                        <div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className={`mr-1 ${item.turnoverRate > 5 ? 'text-green-500' : 'text-gray-500'}`}>
                          {item.turnoverRate > 5 ? <TrendingUp size={16} /> : <TrendingDown size={16} />}
                        </div>
                ...
                {dashboardData.recentDeliveries.map((delivery) => (
                  <tr key={delivery.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <div className="text-xs">{delivery.items} items, {formatCurrency(delivery.value)}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                ...
                {dashboardData.pendingOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <div className="text-xs">{order.items} items, {formatCurrency(order.value)}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs rounded-full font-medium ${
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {dashboardData.vendorPerformance.map((vendor) => (
                <tr key={vendor.vendor} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex text-amber-400">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <svg key={star} className={`w-4 h-4 ${star <= Math.round(vendor.qualityScore) ? 'text-amber-500' : 'text-gray-300'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`text-sm ${vendor.costVariance <= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        <div className="space-y-4">
          <div className="flex items-start p-3 bg-red-50 border border-red-200 rounded-md">
            ...
            <div>
              <p className="font-medium text-red-800">Critical: {dashboardData.summary.outOfStockItems} items out of stock</p>
              <p className="text-sm text-red-700 mt-1">
                Create purchase orders for these items immediately to avoid production delays.
              </p>
              <button className="mt-2 text-sm text-white bg-red-600 hover:bg-red-700 px-3 py-1 rounded" onClick={() => {}}>
                View Items
              </button>
            </div>
          </div>
          ...
          <div className="flex items-start p-3 bg-amber-50 border border-amber-200 rounded-md">
            ...
            <div>
              <p className="font-medium text-amber-800">{dashboardData.summary.lowStockItems} items below reorder point</p>
              <p className="text-sm text-amber-700 mt-1">
                These items need to be ordered soon to maintain adequate inventory levels.
              </p>
              <button className="mt-2 text-sm text-white bg-amber-600 hover:bg-amber-700 px-3 py-1 rounded" onClick={() => {}}>
                Generate POs
              </button>
            </div>
          </div>
          ...
          <div className="flex items-start p-3 bg-blue-50 border border-blue-200 rounded-md">
            ...
            <div>
              ...
              <button className="mt-2 text-sm text-white bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded" onClick={() => {}}>
                View Schedule
              </button>
            </div>
          </div>
          ...
          <div className="flex items-start p-3 bg-green-50 border border-green-200 rounded-md">
            ...
            <div>
              ...
              <button className="mt-2 text-sm text-white bg-green-600 hover:bg-green-700 px-3 py-1 rounded" onClick={() => {}}>
                Optimize Orders
              </button>
            </div>
          </div>
