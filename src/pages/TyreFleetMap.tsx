...
const TyreFleetMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const vehicleGroups = mappingData.reduce((acc, row) => {
  ...
  const filteredVehicles = vehicles.filter(vehicle => {
    ...
    const matchesFilter = filterBy === 'all' || 
      (filterBy === 'complete' && vehicle.activeTyres === vehicle.totalTyres - vehicle.spareTyres) ||
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'mounted': return 'bg-green-100 text-green-800';
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">
            Visual representation of tyre positions across your fleet
          </p>
        </div>
