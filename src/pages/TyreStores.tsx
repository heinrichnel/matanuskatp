...
const TyreStores: React.FC = () => {
  ...
  const [selectedStore, setSelectedStore] = useState<TyreStoreLocation>(TyreStoreLocation.VICHELS_STORE);
  ...
  if (error) {
    return <div className="p-4 text-center text-red-500">Error loading tyre data: {error.message}</div>;
  }
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3">
                        <div>
                          <h4 className="font-semibold">{tyre.brand} {tyre.model}</h4>
                          <p className="text-sm text-gray-600">{tyre.pattern} - {tyre.size.width}/{tyre.size.aspectRatio}R{tyre.size.rimDiameter}</p>
                    ...
                    <div className="text-right">
                      <Badge className={`${
                        tyre.condition.status === 'good' ? 'bg-green-100 text-green-800' :
                        tyre.condition.status === 'warning' ? 'bg-yellow-100 text-yellow-800' :
