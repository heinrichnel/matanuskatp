...
import { AlertOctagon, Calendar, Search, Filter, ChevronRight, Clock, FileText, CheckCircle2 } from 'lucide-react';
...
const DriverViolations: React.FC = () => {
  ...
  const getSeverityBadge = (severity: string) => {
    switch(severity) {
      case 'low':
        return <span className="px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-800">Low</span>;
      case 'medium':
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case 'under-review':
        return <span className="px-2 py-1 text-xs rounded-full bg-purple-100 text-purple-800">Under Review</span>;
      case 'resolved':
          ...
          <CardContent className="p-4">
            <div className="flex flex-col">
              ...
              <p className="text-3xl font-bold text-orange-600">
                {violations.filter(v => v.status === 'pending' || v.status === 'under-review').length}
              </p>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Last Violation</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Common Type</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                ...
                {Object.entries(
                  violations.reduce((acc, violation) => {
                    ...
                    acc[violation.driverId].count++;
                    acc[violation.driverId].violations.push(violation);
                    
                    const currentDate = new Date(violation.date);
                    if (!acc[violation.driverId].latestDate || currentDate > new Date(acc[violation.driverId].latestDate)) {
                      acc[violation.driverId].latestDate = violation.date;
                    }
                ...
                .map(([driverId, data]) => {
                  ...
                  const typeCounts = data.violations.reduce((acc, violation) => {
                    acc[violation.type] = (acc[violation.type] || 0) + 1;
                    return acc;
                  }, {} as Record<string, number>);
                  ...
                  return (
                    <tr key={driverId}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{formatDate(data.latestDate)}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
