...
import { Award, Gift, User, Calendar, Search, ChevronUp, ChevronDown, Star, Trophy, Clock } from 'lucide-react';
...
const mockRewards = [
  ...
  },
  {
    ...
    redeemed: [
      { date: '2023-08-01', points: 300, reward: 'Shopping voucher' }
    ]
  },
  {
    id: 'rew-003',
  ...
  },
  {
    ...
    redeemed: [
      { date: '2023-08-10', points: 300, reward: 'Shopping voucher' }
    ]
  }
];
...
const availableRewards = [
  { id: 'reward-001', name: 'Shopping Voucher', points: 300, description: 'GH₵200 shopping voucher for selected stores', stock: 10 },
  { id: 'reward-002', name: 'Fuel Card Bonus', points: 500, description: 'GH₵350 credit on company fuel card', stock: 5 },
  { id: 'reward-003', name: 'Family Dinner Voucher', points: 500, description: 'Dinner for 4 at selected restaurants', stock: 8 },
  { id: 'reward-004', name: 'Weekend Getaway', points: 1000, description: '2-night stay at selected hotels', stock: 3 },
  { id: 'reward-005', name: 'Smartphone', points: 1200, description: 'Mid-range smartphone', stock: 2 },
  { id: 'reward-006', name: 'Premium Watch', points: 1500, description: 'Quality timepiece', stock: 2 },
  { id: 'reward-007', name: 'Annual Leave Bonus', points: 2000, description: '2 extra days of annual leave', stock: 'Unlimited' },
  { id: 'reward-008', name: 'Flight Tickets', points: 3000, description: 'Return tickets to selected destinations', stock: 1 }
];
...
const DriverRewards: React.FC = () => {
  ...
  const getLevelBadge = (level: string) => {
    switch(level) {
      case 'bronze':
        return <span className="px-2 py-1 text-xs rounded-full bg-amber-100 text-amber-800">Bronze</span>;
      case 'silver':
        return <span className="px-2 py-1 text-xs rounded-full bg-gray-200 text-gray-800">Silver</span>;
      case 'gold':
        return <span className="px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-800">Gold</span>;
      case 'platinum':
        return <span className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">Platinum</span>;
      default:
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button 
          variant="outline" 
          onClick={() => alert('Open the rewards program settings')}
        >
          Program Settings
        </Button>
      </div>
              ...
              .map((reward, index) => (
                <div 
                  ...
                  className={`border rounded-md overflow-hidden ${
                    index === 0 ? 'border-yellow-300' : index === 1 ? 'border-gray-300' : 'border-amber-600'
                  }`}
                >
                ...
                {expandedDriver === reward.driverId && (
                  <div className="border-t px-4 py-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <h4 className="text-sm font-medium text-gray-900 mb-3">Recent Points Earned</h4>
                            ...
                            <div key={`history-${index}`} className="flex items-start space-x-2">
                              ...
                              <div className="flex-1">
                                <div className="flex justify-between items-start">
                                  ...
                                  <div className="text-sm font-medium text-green-600">+{history.points}</div>
                                </div>
                              ...
                              <div key={`redeemed-${index}`} className="flex items-start space-x-2">
                                ...
                                <div className="flex-1">
                                  <div className="flex justify-between items-start">
                                    ...
                                    <div className="text-sm font-medium text-gray-600">-{redeemed.points}</div>
                                  </div>
                                  <div className="text-xs text-gray-500">{formatDate(redeemed.date)}</div>
                                </div>
                              </div>
                            ))}
                          </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    <div className="flex items-center">
                      ...
                      <span>{typeof reward.stock === 'number' ? `${reward.stock} available` : reward.stock}</span>
                    </div>
