...
import ProbeVerificationModal from "../../components/Models/Diesel/AutomaticProbeVerificationModal";
...
// Define the interface here as we need it for type casting
...
const DieselDashboard: React.FC = () => {
  ...
  const enhancedRecords = dieselRecords.map((record) => {
    ...
    const costPerKm =
      !isReeferUnit && distanceTravelled > 0 ? record.totalCost / distanceTravelled : 0;

    // Calculate cost per hour (for reefer units)
    const costPerHour =
      isReeferUnit && record.hoursOperated && record.hoursOperated > 0
        ? record.totalCost / record.hoursOperated
        : 0;
    ...
    const performanceStatus = isReeferUnit
      ? "normal"
      : isWithinTolerance
        ? "normal"
    ...
    const linkedHorse =
      isReeferUnit && record.linkedHorseId
        ? dieselRecords.find((r) => r.id === record.linkedHorseId)
        : undefined;

    // Get linked horse's trip
    const linkedHorseTrip = linkedHorse?.tripId
      ? trips.find((t) => t.id === linkedHorse.tripId)
      : undefined;
  ...
  const handleEdit = (recordId: string) => {
    ...
    if (record) {
      ...
      setEditData({
        ...
        previousKmReading:
          record.previousKmReading !== undefined ? String(record.previousKmReading) : "",
  ...
  const handleSave = (recordId: string) => {
    ...
    if (record) {
      ...
      const distanceTravelled =
        !isReeferUnit && previousKmReading !== undefined
          ? kmReading - previousKmReading
          : record.distanceTravelled;
      const kmPerLitre =
        !isReeferUnit && distanceTravelled && litresFilled > 0
      ...
      // Calculate litres per hour for reefer units
      const litresPerHour =
        isReeferUnit && hoursOperated && hoursOperated > 0
          ? litresFilled / hoursOperated
          : undefined;
  ...
  const handleDelete = async (recordId: string) => {
    ...
    if (
      ...
      try {
        ...
        alert(
          `Error deleting diesel record: ${error instanceof Error ? error.message : "Unknown error"}`
        );
      } finally {
        setIsDeleting(false);
      }
...
6F,2025-01-18,0,,250,19.50,4875,Engen Beitbridge,Peter Farai,Reefer unit refill,ZAR,,true,5.5`;
  ...
  const averageKmPerLitre =
    fleetSummary.totalLitres > 0 && fleetSummary.totalDistance > 0
      ? fleetSummary.totalDistance / fleetSummary.totalLitres
      : 0;
  const averageCostPerKm =
    fleetSummary.totalDistance > 0 ? fleetSummary.totalCost / fleetSummary.totalDistance : 0;
  const averageLitresPerHour =
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-3">
          ...
          <Button icon={<Upload className="w-4 h-4" />} onClick={() => setIsImportModalOpen(true)}>
            Import Diesel CSV
          </Button>
        </div>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <Select
              label="Fleet"
              value={filterFleet}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setFilterFleet(e.target.value)}
              options={[
            ...
            <Select
              ...
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                setFilterProbeStatus(e.target.value)
              }
              options={[
                          ...
                          {editingId === record.id ? (
                            <input
                              ...
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                                setEditData((prev) => ({
                                  ...prev,
                                  previousKmReading: e.target.value,
                                }))
                              }
                            />
                          ) : (
                      ...
                      <div>
                        ...
                        {editingId === record.id ? (
                          <input
                            ...
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                              setEditData((prev) => ({ ...prev, hoursOperated: e.target.value }))
                            }
                          />
                        ) : (
                          <p className="font-medium">
                            {record.hoursOperated?.toFixed(1) || "N/A"} hours
                          </p>
                        )}
                      </div>
                    )}
                          ...
                          {record.linkedHorseInfo && (
                            <div className="text-xs bg-purple-50 p-2 rounded border border-purple-200">
                              ...
                              <p className="text-purple-700 mt-1">
                                Fleet {record.linkedHorseInfo.fleetNumber} -{" "}
      ...
      <DieselDebriefModal
        ...
        records={enhancedRecords.filter((r) => r.requiresDebrief) as unknown as DieselRecord[]}
        norms={dieselNorms}
      />
