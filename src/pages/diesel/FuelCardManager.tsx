                ...
                {filteredCards.length === 0 ? (
                  ...
                  filteredCards.map((card) => (
                    <tr key={card.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{card.fleetNumber || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {getStatusBadge(card.status)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(card.issueDate)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDate(card.expiryDate)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${card.monthlyLimit.toFixed(2)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          ...
                          <div className="w-24 bg-gray-200 rounded-full h-2.5">
                            <div 
                              className={`h-2.5 rounded-full ${
                                card.currentUsage / card.monthlyLimit > 0.9 ? 'bg-red-500' :
                                card.currentUsage / card.monthlyLimit > 0.75 ? 'bg-yellow-500' : 'bg-green-500'
                              }`}
                              style={{ width: `${Math.min(100, (card.currentUsage / card.monthlyLimit) * 100)}%` }}
                            ></div>
                          </div>
                        </div>
                      </td>
