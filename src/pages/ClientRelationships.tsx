  ...
  const handleRemoveRelationship = async (relationshipId: string) => {
    ...
    if (confirm('Are you sure you want to remove this relationship?')) {
      try {
        ...
        alert(`Failed to remove relationship: ${error instanceof Error ? error.message : 'An unknown error occurred'}`);
      }
    }
  };
                ...
                <div className="bg-blue-50 rounded-lg p-8">
                  <div className="flex justify-center items-center">
                    <div className="bg-white rounded-lg shadow-md p-4 border-2 border-blue-500">
                      ...
                      <p className="text-xs text-gray-500 text-center">{selectedClient?.type === 'internal' ? 'Internal' : 'External'}</p>
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mt-8">
                    {relationships.map((rel, index) => {
                      const relatedClientId = rel.source === selectedClientId ? rel.target : rel.source;
                        ...
                        <div key={index} className="flex flex-col items-center">
                          <div 
                            className={`w-full bg-white rounded-lg shadow-md p-4 border ${
                              rel.type === 'parent' ? 'border-purple-500' : 
                              rel.type === 'subsidiary' ? 'border-green-500' :
                              rel.type === 'partner' ? 'border-blue-500' : 'border-red-500'
                            }`}
                          >
                            <p className="font-bold text-center">{relatedClient.name}</p>
                            <p className="text-xs text-gray-500 text-center">{relatedClient.type === 'internal' ? 'Internal' : 'External'}</p>
                            <p className={`text-xs mt-2 text-center ${
                              rel.type === 'parent' ? 'text-purple-600' : 
                              rel.type === 'subsidiary' ? 'text-green-600' :
                              rel.type === 'partner' ? 'text-blue-600' : 'text-red-600'
                            }`}>
                      ...
                      <div key={index} className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                        <div>
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="font-medium">{relatedClient?.name || 'Unknown Client'}</span>
                            <span className={`px-2 py-0.5 rounded-full text-xs ${
                              rel.type === 'parent' ? 'bg-purple-100 text-purple-800' : 
                              rel.type === 'subsidiary' ? 'bg-green-100 text-green-800' :
                              rel.type === 'partner' ? 'bg-blue-100 text-blue-800' : 
          ...
          {availableClients.length === 0 && clients.length > 1 && (
            <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-700">
                This client already has relationships with all other clients in the system.
                Add more clients to create additional relationships.
              </p>
            </div>
          )}
        </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700 mt-1">
                  Define a relationship between {selectedClient?.name} and another client in your system.
                </p>
              </div>
            </div>
          </div>
