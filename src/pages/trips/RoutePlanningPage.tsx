...
const center = { lat: -28.4793, lng: 24.6727 }; // South Africa as default
...
const RoutePlanningPage: React.FC = () => {
  ...
  useEffect(() => {
    if (tripId) {
      ...
      if (tripData) {
        setTrip(tripData);
        // Prefill form if route data exists
        if (tripData.plannedRoute) {
          setOrigin(tripData.plannedRoute.origin);
          setDestination(tripData.plannedRoute.destination);
          setWaypoints(tripData.plannedRoute.waypoints.length > 0 ? tripData.plannedRoute.waypoints : [""]);
  ...
  }, [tripId, getTrip]); // eslint-disable-line
  ...
  const calculateRoute = useCallback(
    async (originValue = origin, destinationValue = destination, waypointsValue = waypoints) => {
      ...
      try {
        ...
        return {
          distance: result.routes[0].legs.reduce((sum, l) => sum + (l.distance?.value || 0), 0) / 1000,
          duration: result.routes[0].legs.reduce((sum, l) => sum + (l.duration?.value || 0), 0) / 60,
          origin: originValue,
          destination: destinationValue,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div>
          {trip && (
            <Button onClick={() => navigate(`/trips/${tripId}`)} variant="outline">
