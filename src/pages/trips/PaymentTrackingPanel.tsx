...
const PaymentTrackingPanel: React.FC<PaymentTrackingPanelProps> = ({ 
  ...
  const daysSinceInvoice = invoice ? 
    Math.floor((new Date().getTime() - new Date(invoice.invoiceDate).getTime()) / (1000 * 3600 * 24)) : 0;
  
  const daysTillDue = invoice ? 
    Math.floor((new Date(invoice.dueDate).getTime() - new Date().getTime()) / (1000 * 3600 * 24)) : 0;
