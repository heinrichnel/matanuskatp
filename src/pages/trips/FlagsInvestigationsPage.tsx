...
// Define FlaggedCost interface for this component
interface FlaggedCost extends CostEntry {
  tripId: string;
...
const FlagsInvestigations: React.FC<FlagsInvestigationsProps> = (props) => {
  ...
  const flaggedCosts = useMemo(
    () => getAllFlaggedCosts(trips) as unknown as FlaggedCost[],
    [trips]
  );

  const filteredCosts = useMemo(() => {
    return flaggedCosts.filter((cost: FlaggedCost) => {
      if (statusFilter && cost.investigationStatus !== statusFilter) return false;
      const trip = trips.find((t: Trip) => t.id === cost.tripId);
  ...
  const handleResolveFlag = async (updatedCost: CostEntry) => {
    try {
      ...
      setFeedback({
        type: "success",
        message: `Flag for ${updatedCost.referenceNumber} resolved.`,
      });
      ...
      if (unresolvedFlagsForTrip.length === 0 && trip) {
        ...
        setFeedback({
          type: "success",
          message: `All flags for Trip ${trip.fleetNumber} resolved. Trip automatically marked as completed.`,
        });
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span>{counts.total} total flags</span>
          </div>
        </div>
      </div>
      ...
      {feedback && (
        <div
          className={`p-4 rounded-md flex items-start space-x-3 ${feedback.type === "success" ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}
        >
      ...
      {filteredCosts.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredCosts.map((cost: FlaggedCost) => {
