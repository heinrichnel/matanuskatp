...
export const createAppError = (
  ...
  if (!options.category) {
    ...
    if (error.name === 'NetworkError' || errorMsg.includes('network') || errorMsg.includes('offline') || errorMsg.includes('connection')) {
      category = ErrorCategory.NETWORK;
    } else if (errorMsg.includes('permission') || errorMsg.includes('access') || errorMsg.includes('not allowed')) {
    ...
    } else if (errorMsg.includes('database') || errorMsg.includes('firestore') || errorMsg.includes('query')) {
  ...
  const attemptOperation = async (): Promise<T> => {
    try {
      ...
      const shouldRetry = 
        ...
        appError.retryable &&
        (!isNetworkError || networkState.isOnline) &&
      ...
      logError(appError.originalError, {
        ...
        severity: isNetworkError && !networkState.isOnline 
