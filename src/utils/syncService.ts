      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          if (this.listeners.onTripUpdate) {
            this.listeners.onTripUpdate({ id: tripId, ...trip });
          }
        }
      },
      (error) => {
        ...
        snapshot.docChanges().forEach((change) => {
          ...
          if (change.type === 'added' || change.type === 'modified') {
            ...
            if (this.listeners.onDieselUpdate) {
              this.listeners.onDieselUpdate({ id: change.doc.id, ...dieselRecord });
            }
          }
        });
      },
        ...
        snapshot.docChanges().forEach((change) => {
          ...
          if (change.type === 'added' || change.type === 'modified') {
            ...
            if (this.listeners.onDriverBehaviorUpdate) {
              this.listeners.onDriverBehaviorUpdate({ id: change.doc.id, ...event });
            }
          }
        });
      },
